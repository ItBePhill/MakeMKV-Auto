...
# pop open the disc drive when ready for a new disc / done
...
def GetInfo(makemkv_path):
    ...
    subpr = subprocess.Popen(args = makemkv_info_args, executable=f"{makemkv_path}\\makemkvcon", stdout=pipefile)
...
def Rip(makemkv_args, makemkv_path):
    ...
    subpr = subprocess.Popen(args = makemkv_args, executable=f"{makemkv_path}\\makemkvcon", shell=False, stdout= subprocess.PIPE, text=True)
    while True:
        line = ""
        message = ""
